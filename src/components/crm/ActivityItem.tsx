...
const activityTypeConfig: Record<string, { icon: typeof StickyNote; color: string; label: string }> = {
  ...
  doc_summary: { icon: FileText, color: "text-status-bienvenida", label: "Resumen" },
};
...
export function ActivityItem({ activity, onUpdate }: ActivityItemProps) {
  ...
  const isEditable = !["status_change", "field_edit", "doc_selected", "doc_summary"].includes(activity.type);
      ...
      <div className="flex-1 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            {editing && isEditable ? (
              <div className="flex items-center gap-1.5">
                <input value={editText} onChange={(e) => setEditText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSaveEdit()} className="flex-1 text-xs py-1 px-2 bg-muted/50 border border-border rounded" autoFocus />
                <button onClick={handleSaveEdit} className="p-1 text-primary"><Check size={12} /></button>
                <button onClick={() => { setEditing(false); setEditText(activity.text); }} className="p-1 text-muted-foreground"><X size={12} /></button>
              </div>
            ) : activity.type === "status_change" && activity.meta ? (
              <div>
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className="text-xs text-muted-foreground">Estado:</span>
                  <span className="text-xs font-medium px-1.5 py-0.5 rounded bg-destructive/10 text-destructive">{activity.meta.fromStatus}</span>
                  <ArrowRight size={10} className="text-muted-foreground" />
                  <span className="text-xs font-medium px-1.5 py-0.5 rounded bg-status-lograr/10 text-status-lograr">{activity.meta.toStatus}</span>
                </div>
                {activity.text && activity.text !== "Estado cambiado" && (
                  <p className="text-[11px] text-foreground/70 mt-0.5 italic">{activity.text.replace("Estado cambiado · ", "")}</p>
                )}
              </div>
              ...
              <pre className="text-[11px] text-foreground whitespace-pre-wrap font-sans leading-relaxed bg-muted/30 rounded-lg p-2 border border-border/50">
                {activity.text}
              </pre>
            ) : (
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <span className={`text-[10px] font-medium px-1 py-0.5 rounded ${config.color} bg-muted/50`}>{config.label}</span>
              <p className="text-[11px] text-muted-foreground font-medium">{activity.author}</p>
              {activity.editedAt && <span className="text-[10px] text-muted-foreground italic">(editado)</span>}
            </div>
          </div>

          <div className="flex items-center gap-1 shrink-0">
            {activity.scheduledAt && (
              <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded flex items-center gap-0.5 ${activity.completed ? "bg-status-lograr/10 text-status-lograr line-through" : "bg-primary/10 text-primary"}`}>
                📅 {activity.scheduledAt}
              </span>
            )}
            ...
            <div className="hidden group-hover:flex items-center gap-0.5 ml-1">
              {isEditable && (
                <button onClick={() => { setEditing(true); setEditText(activity.text); }} className="p-1 rounded hover:bg-muted text-muted-foreground" title="Editar"><Pencil size={11} /></button>
              )}
              <button onClick={() => setShowCommentInput(!showCommentInput)} className="p-1 rounded hover:bg-muted text-muted-foreground" title="Comentar"><MessageCircle size={11} /></button>
            </div>
          </div>
        </div>

        {comments.length > 0 && (
          <button onClick={() => setCommentsOpen(!commentsOpen)} className="flex items-center gap-1 mt-1.5 text-[11px] text-primary hover:text-primary/80">
        ...
        {showCommentInput && (
          <div className="mt-2 space-y-1.5">
            <select value={commentAuthor} onChange={(e) => setCommentAuthor(e.target.value)} className="text-[11px] py-1 px-1.5 bg-muted/50 border border-border rounded">
              {USERS.map((u) => <option key={u} value={u}>{u}</option>)}
            </select>
            <div className="flex gap-1.5">
              <input value={commentText} onChange={(e) => setCommentText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleAddComment()} placeholder="Agregar comentario..." className="flex-1 text-[11px] py-1.5 px-2 bg-muted/50 border border-border rounded-md placeholder:text-muted-foreground" autoFocus />
              <button onClick={handleAddComment} disabled={!commentText.trim()} className="px-2 py-1 rounded-md bg-primary text-primary-foreground text-[11px] font-medium disabled:opacity-40">Enviar</button>
            </div>
          </div>
        )}
      </div>
